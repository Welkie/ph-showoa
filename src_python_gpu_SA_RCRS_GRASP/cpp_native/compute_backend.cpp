...
BaseComputeBackend* create_backend(const Data& data, const std::string& mode) {
    ...
    if (requested == "auto" || requested == "cuda") {
        ...
        if (paper_scale_cpu && data.architecture != "full_gpu") {
        ...
        try {
            ...
            if (data.architecture == "full_gpu") {
                throw std::runtime_error(
                    std::string("architecture full_gpu requires CUDA backend: ") + e.what()
                );
            }
    ...
    if (requested == "cuda") {
        if (data.architecture == "full_gpu") {
            throw std::runtime_error("architecture full_gpu requires a CUDA-enabled build");
        }
    ...
    if (data.architecture == "full_gpu") {
        throw std::runtime_error("architecture full_gpu cannot use the CPU backend");
    }
    return new CpuComputeBackend(data);
}
